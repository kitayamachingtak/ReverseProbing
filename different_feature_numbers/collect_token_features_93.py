#!/usr/bin/env python3
#Token-level feature collection (93-feature configuration).
...
CONFIG = {
    ...
    'neighbor_windows': [2, 3, 5],
    'top_k_values': [5, 10, 20],


    'medical_keywords': [
        'mg', 'diabetes', 'insulin', 'aspirin', 'metoprolol',
        'simvastatin', 'blood', 'pressure', 'glucose', 'therapy',
        'medication', 'dose', 'treatment', 'patient', 'diagnosis',
    ],
}
...
def extract_all_token_features_single_forward(
    ...
    if len(summary_token_ids) > MAX_SUMMARY_TOKENS:
        print(f"    Warning: summary too long ({len(summary_token_ids)} tokens), truncating to {MAX_SUMMARY_TOKENS}")
    ...
    bhc_inputs = tokenizer(f"Brief Hospital Course:\n{bhc_text}\n\nSummary:\n",
                           return_tensors="pt")
    bhc_length = bhc_inputs["input_ids"].shape[1]
    ...
    with torch.no_grad():
        outputs_without = gen_model(
            summary_inputs["input_ids"],
            return_dict=True
        )
        logits_without = outputs_without.logits[0, summary_start-1:summary_start-1+len(summary_token_ids)]
    ...
    for i in range(len(summary_token_ids)):
        ...
        topk_vals = torch.topk(probs, k=10).values
        features['topk_cumulative'] = topk_vals.sum().item()

        sorted_probs, _ = torch.sort(probs, descending=True)
        n = len(sorted_probs)
        index = torch.arange(1, n + 1, dtype=torch.float32).to(probs.device)
        features['gini'] = (2 * torch.sum(index * sorted_probs) / (n * torch.sum(sorted_probs)) - (n + 1) / n).item()

        features['perplexity'] = torch.exp(-log_probs[token_id]).item()
        max_logit = logits.max()
        features['energy'] = -(max_logit + torch.log(torch.sum(torch.exp(logits - max_logit)))).item()
        ...
        features['delta_energy'] = features['energy'] - (-(logits_no.max() + torch.log(torch.sum(torch.exp(logits_no - logits_no.max())))).item())

        for k in CONFIG['top_k_values']:
            topk_probs, topk_indices = torch.topk(probs, k=k)

            rank = 0
            for idx, tid in enumerate(topk_indices):
                if tid == token_id:
                    rank = idx + 1
                    break
            ...
            if token_str and token_str.strip():
                ...
                try:
                    original_emb = embed_model.encode([token_str], show_progress_bar=False)
                    topk_embs = embed_model.encode(topk_tokens, show_progress_bar=False)
                    similarities = cosine_similarity(original_emb, topk_embs)[0]
                    ...
                    for idx, sim in enumerate(similarities):
                        if sim > 0.7:
                            semantic_rank = idx + 1
                            break
                    features[f'semantic_rank_top{k}'] = semantic_rank
        ...
        for window in CONFIG['neighbor_windows']:
            ...
            for j in range(max(0, i-window), min(len(summary_token_ids), i+window+1)):
                if j != i and j < len(logits_with):
                    neighbor_logits = logits_with[j]
                    neighbor_probs_dist = torch.nn.functional.softmax(neighbor_logits, dim=-1)
                    neighbor_tid = summary_token_ids[j]
                    neighbor_probs.append(neighbor_probs_dist[neighbor_tid].item())

                    neighbor_token = tokenizer.decode([neighbor_tid]).strip().lower()
                    if any(kw in neighbor_token for kw in CONFIG['medical_keywords']):
                        medical_count += 1

            if neighbor_probs:
                neighbor_avg = np.mean(neighbor_probs)
                neighbor_std = np.std(neighbor_probs)
            else:
                neighbor_avg = features['current_prob']
                neighbor_std = 0.0
            ...
            features[f'relative_isolation_w{window}'] = (features['current_prob'] - neighbor_avg) / (neighbor_std + 1e-10)

            window_size = min(len(summary_token_ids), i+window+1) - max(0, i-window)
            features[f'medical_density_w{window}'] = medical_count / window_size if window_size > 0 else 0

        features['is_medical'] = float(any(kw in token_str for kw in CONFIG['medical_keywords']))
        ...
        for idx in range(len(layers)-1):
            ...
            if l1 < num_layers and l2 < num_layers and actual_idx < hidden_states_with[l1].shape[1]:
                ...
                features[f'layer_cosine_l{l1}_to_l{l2}'] = torch.nn.functional.cosine_similarity(
        ...
        if attentions_with:
            for layer_idx, head_idx in CONFIG['uncertainty_heads']:
                if layer_idx < len(attentions_with) and actual_idx < attentions_with[layer_idx].shape[2]:
                    ...
                    features[f'attn_entropy_l{layer_idx}_h{head_idx}'] = attn_entropy
...
def get_baseline_logit_simple(token_id, model, tokenizer):
    input_ids = torch.tensor([[tokenizer.bos_token_id if tokenizer.bos_token_id else 1, token_id]]).to(model.device)
...
def build_vocabulary_stats(data_list, tokenizer):
    ...
    for data in data_list:
        ...
        for token_id in unique_tokens:
            doc_contains_token[token_id] = doc_contains_token.get(token_id, 0) + 1

    total_docs = len(data_list)
    token_idf = {}
    for token_id, df in doc_contains_token.items():
        token_idf[token_id] = math.log(total_docs / (df + 1))

    return token_freq, token_idf


def process_dataset(data_list, gen_model, tokenizer, embed_model,
                   token_freq, token_idf, baseline_cache, dataset_name):
...
def main():
    print("="*80)
    print("OPTIMIZED: ALL TOKEN FEATURES IN ONE PASS")
    print("="*80)
    ...
    if CONFIG['use_multi_gpu'] and n_gpus > 1:
        ...
        if CONFIG['max_memory_per_gpu']:
            max_memory = {i: CONFIG['max_memory_per_gpu'] for i in range(n_gpus)}
            max_memory['cpu'] = '30GB'
            print(f"  Max memory per GPU: {CONFIG['max_memory_per_gpu']}")
        else:
            max_memory = None
    else:
    ...
    if hasattr(gen_model, 'hf_device_map'):
        for name, device in list(gen_model.hf_device_map.items())[:5]:
            print(f"    {name}: {device}")
        if len(gen_model.hf_device_map) > 5:
            print(f"    ... and {len(gen_model.hf_device_map) - 5} more layers")

    apply_preset(CONFIG, gen_model, script="93")
    ...
    print(f"\n[3] Splitting dataset ({CONFIG['train_test_split']*100:.0f}% train / {(1-CONFIG['train_test_split'])*100:.0f}% test)...")
    ...
    print(f"Saved {len(train_features)} train vectors")
    print(f"  Features: {len(train_features[0])}")
    print(f"  Positive: {np.sum(train_labels)} ({np.mean(train_labels)*100:.2f}%)")

    print(f"\n[7] Collecting TEST features...")
    test_features, test_labels = process_dataset(
        test_data, gen_model, tokenizer, embed_model,
        token_freq, token_idf, baseline_cache, "TEST"
    )

    print(f"\n[8] Saving test features to {CONFIG['test_output']}...")
    test_output = {
    ...
    print(f"Saved {len(test_features)} test vectors")
    print(f"  Features: {len(test_features[0])}")
    print(f"  Positive: {np.sum(test_labels)} ({np.mean(test_labels)*100:.2f}%)")
