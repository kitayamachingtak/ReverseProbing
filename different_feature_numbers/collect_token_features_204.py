...
#Token-level feature collection (204-feature configuration).
#Extends the 120-feature set with expanded context-contrast features (KL/JS divergence, rank shift, top-k overlap), one-hot medical entity-type flags, PMI-based grounding signals, entropy decomposition, medical prior decomposition, attention drift, and attention rollout. 
...
def extract_all_token_features_single_forward(
    ...
    for i in range(len(summary_token_ids)):
        ...
        energy_no = -(logits_no.max() + torch.log(torch.sum(torch.exp(logits_no - logits_no.max())))).item()
        features['delta_energy']  = features['energy'] - energy_no

        kl_with_no = (probs * torch.log(probs / (probs_no + 1e-10) + 1e-10)).sum().item()
        kl_no_with = (probs_no * torch.log(probs_no / (probs + 1e-10) + 1e-10)).sum().item()
        m = 0.5 * (probs + probs_no)
        js_div = (
            0.5 * (probs    * torch.log(probs    / (m + 1e-10) + 1e-10)).sum().item() +
            0.5 * (probs_no * torch.log(probs_no / (m + 1e-10) + 1e-10)).sum().item()
        )
        features['kl_with_vs_no'] = kl_with_no
        features['kl_no_vs_with'] = kl_no_with
        features['js_divergence'] = js_div

        def _rank(p, tid):
            return (torch.argsort(p, descending=True) == tid).nonzero(as_tuple=True)[0].item() + 1
        rank_with = _rank(probs, token_id)
        rank_no   = _rank(probs_no, token_id)
        features['rank_with_bhc']    = rank_with
        features['rank_without_bhc'] = rank_no
        features['delta_rank']       = rank_no - rank_with

        topk_with_ids = set(torch.topk(probs,    k=10).indices.tolist())
        topk_no_ids   = set(torch.topk(probs_no, k=10).indices.tolist())
        overlap = len(topk_with_ids & topk_no_ids)
        features['topk_overlap'] = overlap / 10.0
        features['topk_jaccard'] = overlap / len(topk_with_ids | topk_no_ids)

        top2_no = torch.topk(probs_no, k=2).values
        features['delta_margin']    = features['margin'] - (top2_no[0] - top2_no[1]).item()
        features['prob_vs_no_max']  = probs[token_id].item() - probs_no.max().item()
        ...
        if ann:
            etype  = ann.get('entity_type', 'O')
            source = ann.get('source', 'vocab')
            features['is_medical']         = 1.0
            features['ner_entity_type']    = _encode_entity_type(etype)
            features['medical_confidence'] = {'ner+vocab': 1.0, 'ner': 0.85, 'vocab': 0.6}.get(source, 0.5)
            features['ner_is_chemical']  = float(etype == 'CHEMICAL')
            features['ner_is_disease']   = float(etype == 'DISEASE')
            features['ner_is_gene']      = float(etype == 'GENE_OR_GENE_PRODUCT')
            features['ner_is_cancer']    = float(etype == 'CANCER')
            features['ner_is_anatomy']   = float(etype in ('ORGAN', 'TISSUE', 'CELL_TYPE',
                                                            'ORGANISM_SUBSTANCE',
                                                            'MULTI-TISSUE_STRUCTURE'))
            features['ner_is_pathology'] = float(etype == 'PATHOLOGICAL_FORMATION')
            features['ner_is_high_risk'] = float(etype in ('CHEMICAL', 'SIMPLE_CHEMICAL', 'CANCER'))
        else:
            features['is_medical']         = 0.0
            features['ner_entity_type']    = 0
            features['medical_confidence'] = 0.0
            features['ner_is_chemical']    = 0.0
            features['ner_is_disease']     = 0.0
            features['ner_is_gene']        = 0.0
            features['ner_is_cancer']      = 0.0
            features['ner_is_anatomy']     = 0.0
            features['ner_is_pathology']   = 0.0
            features['ner_is_high_risk']   = 0.0
        ...
        lp_with  = log_probs[token_id].item()
        lp_no    = log_probs_no[token_id].item()
        lp_prior = math.log(baseline['prob'] + 1e-10)

        pmi = lp_with - lp_no
        pmi_vs_prior = lp_with - lp_prior

        features['pmi']                  = pmi
        features['pmi_vs_prior']         = pmi_vs_prior
        features['npmi'] = pmi / (-lp_with + 1e-10)

        features['_etype_for_cpmi'] = ann.get('entity_type', 'O') if ann else 'O'

        H_total  = entropy
        H_no_bhc = entropy_no
        H_prior  = baseline['entropy']

        bhc_info_gain     = H_no_bhc - H_total
        ctx_info_gain     = H_prior  - H_no_bhc
        total_info_gain   = H_prior  - H_total

        features['bhc_info_gain']   = bhc_info_gain
        features['ctx_info_gain']   = ctx_info_gain
        features['total_info_gain'] = total_info_gain
        features['bhc_contribution_ratio'] = bhc_info_gain / (total_info_gain + 1e-10)
        features['bhc_info_gain_norm'] = bhc_info_gain / (H_prior + 1e-10)

        p_prior   = baseline['prob']
        p_no_bhc  = probs_no[token_id].item()
        p_current = probs[token_id].item()

        features['halluc_risk_ratio']      = p_current / (p_prior + 1e-10)
        features['patient_specificity']    = p_current - p_prior
        features['context_reliance_score'] = (p_current - p_no_bhc) / (p_no_bhc + 1e-10)
        features['prob_dominance_order']   = int(np.argmax([p_prior, p_no_bhc, p_current]))
        features['patient_specificity_norm'] = (p_current - p_prior) / (p_prior + 1e-10)
        ...
        if attentions_with:
            num_attn_layers = len(attentions_with)

            for layer_idx, head_idx in CONFIG['uncertainty_heads']:
                if layer_idx < num_attn_layers and actual_idx < attentions_with[layer_idx].shape[2]:
            ...
            attn_layers = [l for l in CONFIG['attention_layers'] if l < num_attn_layers]
            for head_idx in CONFIG['drift_heads']:
                for k in range(len(attn_layers) - 1):
                    l1, l2 = attn_layers[k], attn_layers[k + 1]
                    if (head_idx < attentions_with[l1].shape[1] and
                            actual_idx < attentions_with[l1].shape[2] and
                            actual_idx < attentions_with[l2].shape[2]):
                        a1 = attentions_with[l1][0, head_idx, actual_idx, :]
                        a2 = attentions_with[l2][0, head_idx, actual_idx, :]
                        p  = a1 / (a1.sum() + 1e-10)
                        q  = a2 / (a2.sum() + 1e-10)
                        kl = (p * torch.log(p / (q + 1e-10) + 1e-10)).sum().item()
                        features[f'attn_drift_l{l1}_l{l2}_h{head_idx}'] = kl

            rollout_checkpoints = set(
                l for l in CONFIG['rollout_checkpoints'] if l < num_attn_layers)
            seq_len = attentions_with[0].shape[3]
            rollout = None
            max_rollout_layer = min(max(rollout_checkpoints) + 1, num_attn_layers)
            for l in range(max_rollout_layer):
                attn_avg = attentions_with[l][0].mean(dim=0)
                eye      = torch.eye(seq_len, device=attn_avg.device, dtype=attn_avg.dtype)
                attn_avg = 0.5 * attn_avg + 0.5 * eye
                attn_avg = attn_avg / (attn_avg.sum(dim=-1, keepdim=True) + 1e-10)
                rollout  = attn_avg if rollout is None else torch.matmul(rollout, attn_avg)

                if l in rollout_checkpoints and actual_idx < rollout.shape[0]:
                    row = rollout[actual_idx]
                    row_norm = row / (row.sum() + 1e-10)
                    features[f'rollout_to_bhc_l{l}']     = row[:bhc_length].sum().item()
                    features[f'rollout_entropy_l{l}']    = (
                        -(row_norm * torch.log(row_norm + 1e-10)).sum().item()
                    )
                    features[f'rollout_max_weight_l{l}'] = row.max().item()
...
def process_dataset(data_list, gen_model, tokenizer, embed_model,
    ...
    print(f"  Computing CPMI (conditional PMI by entity type)...")
    from collections import defaultdict

    etype_pmis = defaultdict(list)
    for feat in all_features:
        etype = feat.get('_etype_for_cpmi', 'O')
        etype_pmis[etype].append(feat.get('pmi', 0.0))

    etype_stats = {}
    for etype, vals in etype_pmis.items():
        arr = np.array(vals)
        etype_stats[etype] = (arr.mean(), arr.std() + 1e-10)

    for feat in all_features:
        etype = feat.get('_etype_for_cpmi', 'O')
        mu, sigma = etype_stats.get(etype, (0.0, 1.0))
        feat['cpmi'] = (feat.get('pmi', 0.0) - mu) / sigma
        feat.pop('_etype_for_cpmi', None)
...
def main():
    ...
    apply_preset(CONFIG, gen_model, script="204")
